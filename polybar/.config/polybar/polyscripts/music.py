...
# (int) : Length of media info string.
# If length of string exceedes this value, the text will scroll.
# Default value is 20
message_display_len:int = 30

# Font index of polybar. this value should be 1 more than
# the font value specified in polybar config.
font_index:int = 1

# Update speed of the text in seconds.
update_delay:float = 0.3

# List of chars containing previous, play, pause, next glyphs
# for media controls in respective order
control_chars:List = ['','','','']

# (dict) : dict of char icons to display as prefix.
# If player name is available as key, then use the
# corressponding icon, else default key value.
# example:
display_player_prefix:Dict = {"spotify":  '', "firefox":  '', "default":  ''}

# (list) : list of metadata fields based on mpris sepecification.
# For more details/ field names, refer
# [mpris sepecification](https://www.freedesktop.org/wiki/Specifications/mpris-spec/metadata/)
metadata_fields:List = ["xesam:title", "xesam:artist"]

# (char) : separator for metadata fields
metadata_separator:str = "-"

# (bool) : Hide text when no player is available?
# True disables the output for no players.
